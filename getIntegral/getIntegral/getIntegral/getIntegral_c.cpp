...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    ...
    {
        n1x = vertices[int(faces[i]) * 3 - 3];
        n1y = vertices[int(faces[i]) * 3 - 2];
        n1z = vertices[int(faces[i]) * 3 - 1];
        n2x = vertices[int(faces[1 * face_num + i]) * 3 - 3];
        n2y = vertices[int(faces[1 * face_num + i]) * 3 - 2];
        n2z = vertices[int(faces[1 * face_num + i]) * 3 - 1];
        n3x = vertices[int(faces[2 * face_num + i]) * 3 - 3];
        n3y = vertices[int(faces[2 * face_num + i]) * 3 - 2];
        n3z = vertices[int(faces[2 * face_num + i]) * 3 - 1];
