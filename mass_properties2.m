function [mass, cm, inertia] = mass_properties2(node_xyz, face_node, face_num)
    global masstime;
    start = tic;
    ...
    for i = 1 : face_num
        ...
        d = cross(node2 - node1, node3 - node1);
        % compute integral terms
        [f, g] = subExpressions([node1,node2,node3]);
...
function [f, g] = subExpressions(w)
    ...
    g = f(:, 2) * ones(1, 3) + w .* ( w + f(:, 1) * ones(1, 3));
%     g(1) = f(2) + w(1) * (f(1) + w(1));
%     g(2) = f(2) + w(2) * (f(1) + w(2));
%     g(3) = f(2) + w(3) * (f(1) + w(3));
end
