...
public: //--------------------------------------------------- memory management
    ...
    void garbage_collection()
	{
        ...
        for(Vertex_iterator vi = vertices_begin(); vi != vertices_end(); ++vi)
        {
            ...
            if(vidx - 1 < v.idx())
            {
                for(unsigned int i = 0;i < this->map_2skel.size();i++)
                {
                    if(this->map_2skel[i] == v.idx())
                        this->map_2skel[i] = vidx - 1;
                }
            }else if(vidx - 1 > v.idx())
                qDebug()<<"ERROR! cur_index > pre_index!";
            //end
        }
        //add
        //qDebug("pre_index = %d\ncur_index = %d\n",pre_index,cur_index);
...
public: //------------------------------------------ geometry-related functions
	...
	void print_stats()
	{
        ...
        qDebug("\nNumber of vertices \t= %d \n", n_vertices());
        qDebug("Number of total vertices \t= %d \n", vertices_size());
        qDebug("Number of faces \t= %d \n", n_faces());
        qDebug("Number of edges \t= %d \t (out of %d) \n", n_edges(), edges_size());
        unsigned int t = 0;
        for(unsigned int i = 0;i < map_2skel.size();i++)
            if(t < map_2skel[i])t = map_2skel[i];
        qDebug("Max of map_2skel: %d\n\n",t);
    }
...
protected: //------------------------------------------------------- private data
    ...
    //add
public:
    std::vector<unsigned int>map_2skel;
    //end
};
