...
void surfacemesh_io_obj::save(SurfaceMeshModel* mesh,QString path) {
    ...
    //add
    for(unsigned int i = 0;i < mesh->map_2skel.size();i++)
    {
        fprintf(fid,"#map %d %d\n",i+1,mesh->map_2skel[i]+1);
    }
    //end
    ...
    //add
    qDebug("Vertex Total:%d",mesh->vertices_size());
    qDebug("Vertex in Mesh:%d",mesh->n_vertices());
    //end
}
