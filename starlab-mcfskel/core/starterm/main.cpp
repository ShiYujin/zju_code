...
int main(int argc, char *argv[]){ 
    //add
    //filter name:<Normalize Mesh><Voronoi based MAT><SurfaceMesh to Skeleton><>
    argv[argc++] = "--filter=Voronoi based MAT";
//    argv[argc++] = "--filter=MCF Skeletonization";
//    argv[argc++] = "--filter=SurfaceMesh to Skeleton";
    argv[argc++] = "E:/Project/zju_code/triangleMesh/banana_obj_copy1.obj";
    argv[argc++] = "--save";
    argv[argc] = NULL;
    //end
    try{
        ...
        //add
        parser->executeFilter = "MCF Skeletonization";
        for(int i = 0;i < 10;i++)
        {
            foreach(Model* model, document->models()){
                starlab->executeFilter(model, parser->executeFilter);
        ...
        qDebug()<<"MCF Skeletonization finished";
        parser->executeFilter = "SurfaceMesh to Skeleton";
        foreach(Model* model, document->models()){
            starlab->executeFilter(model, parser->executeFilter);
        }
        qDebug()<<"SurfaceMesh to Skeleton finished";
        //end
